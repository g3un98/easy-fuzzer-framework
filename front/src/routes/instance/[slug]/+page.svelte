...
<div class="p-4 flex flex-col text-white gap-4">
    ...
    <div class="flex gap-4">
        {#if State.Code == 80}
            <button on:click={() => startInstance()} class="px-2 py-0.5 rounded-lg bg-amber-400 text-black">Start</button>
        {/if}
        {#if State.Code == 16}
            <button on:click={() => stopInstance()} class="px-2 py-0.5 rounded-lg bg-amber-400 text-black">Stop</button>
        {/if}
        {#if State.Code == 80}
            <button on:click={() => deleteInstance()} class="px-2 py-0.5 rounded-lg bg-amber-400 text-black">Delete</button>
        {/if}
    </div>
</div>
